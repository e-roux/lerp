...
def col2int(n):
    ...
    Note
    ----------
    See from get_column_letter() in openpyxl.utils.cell
    """
...
class xlsx(object):
    ...
    def get_range(self, ws=None, r=None, named_range=None,
                              header=True, transpose=False, columns = None,
                              index=None, lerp=False):
...
def save_style(styleName):
    ...
    stylelibDir = '\\'.join([os.path.normpath(matplotlib.get_configdir()), "stylelib"])

    # Check if stylelib path exists
    if not os.access(stylelibDir, os.R_OK):
        os.mkdir(stylelibDir)

    with codecs.open('\\'.join([stylelibDir, os.path.basename(os.path.splitext(styleName)[0]) + ".mplstyle "]),
                     "w", "utf-8") as f:

        for k in sorted(plt.rcParams):
            if k.split(".")[0] not in ["animation", "backend", "datapath", "docstring","examples", "keymap",
                                       "ps", "svg", "tk", "timezone", "toolbar", "verbose", "webagg"]:
                ...
                if v is None:
                    ...
                    v_rc = "cycler('color', [{}])".format(",".join("'{}'".format(str(_v).replace('#',''))
