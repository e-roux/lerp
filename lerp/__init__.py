...
class Mesh2d(mesh):
    ...
    Instantiation by giving (x, y) parameters or by loading a csv-file.
...
#   def __next__(self):
    def __repr__(self):
        return 'x = {}\ny = {}'.\
        format(self.x.__repr__(), self.y.__repr__())
    ...
    def push(self, x=None, y=None):
        ...
        y: single numeric, array, numpy.array
        ...
        if x is not None and y is not None:
            ...
            try:
                ...
                self.y = self.y.push(y, unique=False)[indices]
            except:
                print("All elements already in orginal array")

        self._sort()

    def apply(self, f, axis="y", inplace=False):
    ...
    def min(self, x=False):
        """Returns the min value of the y-axis.
        If x is set to True, returns a tuple containing
        (min(y), x where y is min)
        ...
        if x is True:
            _x = self.y.argmin()
            return (self.x[_x], self.y[_x])
        return min(self.y)

    def max(self, x=False):
        """Returns the max value of the y-axis.
        If x is set to True, returns a tuple containing
        (max(y), x where y is max)
    ...
    def mean(self, *args, **kwargs):
        return self.y.mean(*args, **kwargs)

    def median(self, *args, **kwargs):
        return self.y.median(*args, **kwargs)

    def interpolate(self, x, assume_sorted=False, *args, **kwargs):
        """Purpose of this method is to return a linear interpolation
        of a y vector for an unknown value x. If the targeted value
        is out of the x range, the returned y-value is the first,
        resp. the last y-value.
        ...
        try:
            if isinstance(x, (np.ndarray, list, range)):
                return interp2d(x, self.x, self.y)

            else:
                return interp2d(np.array([x]), self.x, self.y)[0]
        except:
...
#        self._steps.__call__ = self._steps.step
    ...
    def to_csv(self, fileName=None, nbreDecimales=2):
        ...
        with open(fileName, 'w') as f:
            ...
            writer.writerow([self.x.label, self.y.label])
            # writes units
            writer.writerow([self.x.unit, self.y.unit])

            buf = np.vstack([self.x.transpose(), np.round(self.y.transpose(),
                             nbreDecimales)])
    ...
    def T(self):
        myObj = deepcopy(self)
        myObj.y, myObj.x = myObj.x, myObj.y
        myObj._sort()
        return myObj
       ...
       for power, coeff in reversed(list(enumerate(reversed(self.p.coeffs)))):
           ...
           if coeff == 1:
               result.append(f0.format(j, coeff,
                                       str(power).translate(tr) if html \
                                       is True \
                                       else "^{}".format(power)))
           elif coeff != 0:
...
class PolyMesh3d(object):
    ...
    def push(self, y, p):
        ...
        shape = (len(self.x), len(self.y)) #This will be some pre-determined size
...
#                             x_label=self.x.label, x_unit=self.x.unit,
#                             y_label=self.y.label, y_unit=self.y.unit,
#                             unit=self.z.unit, label=self.z.label)
#                return res
